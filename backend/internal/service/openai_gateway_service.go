...
func (s *OpenAIGatewayService) openAIWSSchedulerWeights() config.GatewayOpenAIWSSchedulerScoreWeights {
	if s != nil && s.cfg != nil {
		weights := s.cfg.Gateway.OpenAIWS.SchedulerScoreWeights
		sum := weights.Priority + weights.Load + weights.Queue + weights.ErrorRate + weights.TTFT
		if sum > 0 {
			return weights
		}
	}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if model, ok := reqBody["model"].(string); ok {
		if !compactMapped {
			resolved, resolveErr := resolveOpenAIUpstreamModelOrFailover(account, model)
			if resolveErr != nil {
