...
func TestForwardAsAnthropic_UsesMappedModelReasoningEffort(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_mapped_effort"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	result, err := svc.ForwardAsAnthropic(context.Background(), c, account, body, "", "gpt-5.5", "xhigh")
	require.NoError(t, err)
	require.NotNil(t, result)
