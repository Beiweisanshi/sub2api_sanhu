...
<template>
 ...
 <div class="min-h-0 flex-1 overflow-auto border-b border-gray-200">
 <table class="w-full border-separate border-spacing-0">
 <thead class="sticky top-0 z-10 bg-gray-50">
 <tr>
 <th class="border-b border-gray-200 px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">
 ...
 <th class="border-b border-gray-200 px-4 py-2.5 text-right text-[11px] font-bold uppercase tracking-wider text-gray-500">
 ...
 <span class="truncate font-mono text-[11px] text-gray-700">
 ...
 <!-- Platform -->
 <td class="whitespace-nowrap px-4 py-2">
 <span class="inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-bold uppercase text-gray-600">
 {{ log.platform || '-' }}
 </span>
 </td>
 ...
 <span class="flex items-center gap-1 truncate font-mono text-[11px] text-gray-700">
 <span class="truncate">{{ log.requested_model }}</span>
 <span class="flex-shrink-0 text-gray-400">→</span>
 <span class="truncate text-primary-600">{{ log.upstream_model }}</span>
 </span>
 </el-tooltip>
 </template>
 <template v-else>
 <span v-if="displayModel(log)" class="truncate font-mono text-[11px] text-gray-700" :title="displayModel(log)">
 {{ displayModel(log) }}
 </span>
 <span v-else class="text-xs text-gray-400">-</span>
 </template>
 ...
 <span class="max-w-[100px] truncate text-xs font-medium text-gray-900">
 {{ log.group_name || '-' }}
 </span>
 </el-tooltip>
 <span v-else class="text-xs text-gray-400">-</span>
 ...
 class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-bold text-gray-600"
 >
 ...
 <p class="truncate text-[11px] font-medium text-gray-600" :title="log.message">
 ...
 <button type="button" class="text-primary-600 hover:text-primary-700 text-xs font-bold" @click="emit('openErrorDetail', log.id)">
...
function getTypeBadge(log: OpsErrorLog): { label: string; className: string } {
 ...
 return { label: t('admin.ops.errorLog.typeUpstream'), className: 'bg-red-50 text-red-700 ring-red-600/20 ' }
 }
 if (phase === 'request' && owner === 'client') {
 return { label: t('admin.ops.errorLog.typeRequest'), className: 'bg-amber-50 text-amber-700 ring-amber-600/20 ' }
 }
 if (phase === 'auth' && owner === 'client') {
 return { label: t('admin.ops.errorLog.typeAuth'), className: 'bg-primary-50 text-primary-700 ring-primary-600/20 ' }
 }
 if (phase === 'routing' && owner === 'platform') {
 return { label: t('admin.ops.errorLog.typeRouting'), className: 'bg-gray-50 text-gray-700 ring-gray-600/20 ' }
 }
 if (phase === 'internal' && owner === 'platform') {
 return { label: t('admin.ops.errorLog.typeInternal'), className: 'bg-gray-100 text-gray-800 ring-gray-600/20 ' }
 }

 const fallback = phase || owner || t('common.unknown')
 return { label: fallback, className: 'bg-gray-50 text-gray-700 ring-gray-600/10 ' }
}
...
function getStatusClass(code: number): string {
 ...
 if (code === 429) return 'bg-gray-50 text-gray-700 ring-gray-600/20 '
