...
import { CLAUDE_CHART_SERIES, CLAUDE_CHART_TEXT, CLAUDE_CHART_TOOLTIP } from '@/utils/chartTheme'
...
const categories = computed<ErrorCategory[]>(() => {
 ...
 if (upstream > 0) out.push({ label: t('admin.ops.upstream'), count: upstream, color: colors.value.clay })
 if (client > 0) out.push({ label: t('admin.ops.client'), count: client, color: colors.value.primary })
 if (system > 0) out.push({ label: t('admin.ops.system'), count: system, color: colors.value.wood })
