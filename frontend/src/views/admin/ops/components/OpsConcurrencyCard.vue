...
<template>
 ...
 <!-- 错误提示 -->
 <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl bg-red-50 p-2.5 text-xs text-red-600">
 {{ errorMessage }}
 </div>
 ...
 class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-gray-200 text-sm text-gray-500"
 >
 {{ t('admin.ops.concurrency.disabledHint') }}
 </div>

 <!-- 数据展示区域 -->
 <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200">
 <!-- 维度标题栏 -->
 ...
 <span class="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold text-gray-700">
 ...
 <span class="font-bold text-primary-600">{{ row.available_accounts }}</span
 >/{{ row.total_accounts }}
 </span>
 ...
 class="rounded-full bg-gray-100 px-1.5 py-0.5 font-semibold text-gray-700"
 >
 {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue }) }}
 </span>
 </div>
 ...
 class="inline-flex items-center gap-1 rounded bg-primary-100 px-1.5 py-0.5 text-[10px] font-medium text-primary-700"
 >
