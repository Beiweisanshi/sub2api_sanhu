...
const levelBadgeClass = (level: string) => {
 ...
 if (v === 'debug') return 'bg-gray-100 text-gray-700 '
...
<template>
 ...
 <div>
 <h3 class="text-sm font-bold text-gray-900">系统日志</h3>
 <p class="mt-1 text-xs text-gray-500">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
 </div>
 <div class="flex flex-wrap items-center gap-2 text-xs">
 <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
 <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700">写入 {{ health.written_count }}</span>
 <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700">丢弃 {{ health.dropped_count }}</span>
 <span class="rounded-md bg-red-100 px-2 py-1 text-red-700">失败 {{ health.write_failed_count }}</span>
 </div>
 </div>

 <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3">
 <div class="mb-2 flex items-center justify-between">
 <div class="text-xs font-semibold text-gray-700">运行时日志配置（实时生效）</div>
 ...
 <p v-if="health.last_error" class="mt-2 text-xs text-red-600">最近写入错误：{{ health.last_error }}</p>
 </div>
