...
// 方案 A 配色优化：健康分颜色恢复绿/黄/红的真实语义，原先高分 90+ 被标成橙色 #C1573A 是明显 bug
const healthScoreColor = computed(() => {
 if (isSystemIdle.value) return '#9CA3AF' // gray-400
 const score = healthScoreValue.value
 if (score == null) return '#9CA3AF'
 if (score >= 90) return '#10B981' // emerald-500 (green)
...
const dbMiddleClass = computed(() => {
 if (systemMetrics.value?.db_ok === false) return 'text-orange-600 '
 if (dbUsagePercent.value != null) {
 if (dbUsagePercent.value >= 90) return 'text-orange-600 '
 if (dbUsagePercent.value >= 70) return 'text-yellow-600 '
 return 'text-primary-600 '
 }
 if (systemMetrics.value?.db_ok === true) return 'text-primary-600 '
...
const redisMiddleClass = computed(() => {
 if (systemMetrics.value?.redis_ok === false) return 'text-orange-600 '
 if (redisUsagePercent.value != null) {
 if (redisUsagePercent.value >= 90) return 'text-orange-600 '
 if (redisUsagePercent.value >= 70) return 'text-yellow-600 '
 return 'text-primary-600 '
 }
 if (systemMetrics.value?.redis_ok === true) return 'text-primary-600 '
 return 'text-gray-900 '
})
...
<template>
 <div :class="['flex flex-col gap-4 rounded-3xl bg-white shadow-sm ring-1 ring-gray-900/5 ', props.fullscreen ? 'p-8' : 'p-6']">
 <!-- Top Toolbar -->
 ...
 v-if="!props.fullscreen"
 type="button"
 class="flex h-8 items-center gap-1.5 rounded-lg bg-primary-100 px-3 text-xs font-bold text-primary-700 transition-colors hover:bg-primary-200"
 ...
 <div :class="['rounded-2xl bg-gray-50 lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
 <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
 <!-- 1) Health Score -->
 <div
 class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-white/60 md:border-r md:border-gray-200 md:pr-6"
 >
 ...
 >
 <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5">
 ...
 <svg v-else class="h-4 w-4 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
 <path
 fill-rule="evenodd"
 ...
 <div v-if="item.action" class="mt-1 text-[11px] text-primary-600 flex items-center gap-1">
 ...
 <!-- Card 1: Requests -->
 <div class="rounded-2xl bg-gray-50 p-4" style="order: 1;">
 ...
 <div class="rounded-2xl bg-gray-50 p-4" style="order: 4;">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-1">
 ...
 <!-- Card 5: TTFT -->
 <div class="rounded-2xl bg-gray-50 p-4" style="order: 5;">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-1">
 ...
 <div class="rounded-2xl bg-gray-50 p-4" style="order: 3;">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-1">
 ...
 <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-500 hover:underline" type="button" @click="openErrorDetails('request')">
 {{ t('admin.ops.requestDetails.details') }}
 </button>
 </div>
 ...
 <div class="rounded-2xl bg-gray-50 p-4" style="order: 6;">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-1">
 ...
 <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-500 hover:underline" type="button" @click="openErrorDetails('upstream')">
 {{ t('admin.ops.requestDetails.details') }}
 </button>
 </div>
 ...
 <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-500 hover:underline" type="button" @click="openJobsDetails">
 {{ t('admin.ops.requestDetails.details') }}
 </button>
 </div>
 ...
 <div
 v-if="hb.last_error"
 class="mt-3 rounded-lg bg-orange-50 p-2 text-xs text-orange-700"
 >
 {{ hb.last_error }}
 </div>
 </div>
 ...
 type="datetime-local"
 class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
 />
 </div>
 <div>
 <label class="block text-sm font-medium text-gray-700 mb-1">
