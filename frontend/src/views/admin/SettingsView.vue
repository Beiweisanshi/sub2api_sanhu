...
<template>
 ...
 <div
 v-if="newAdminApiKey"
 class="space-y-3 rounded-lg border border-primary-200 bg-primary-50 p-4"
 >
 ...
 <code
 class="flex-1 select-all break-all rounded border border-primary-300 bg-white px-3 py-2 font-mono text-sm"
 >
 {{ newAdminApiKey }}
 </code>
 <button
 ...
 <span class="text-xs text-gray-400">{{ t('admin.settings.betaPolicy.commonPatterns') }}:</span>
 <button
 ...
 <div v-if="webSearchConfig.providers.length === 0" class="rounded-lg border border-dashed border-gray-300 p-4 text-center text-sm text-gray-400">
 ...
 <span v-if="!expandedProviders[pIdx] && provider.api_key_configured" class="text-xs text-primary-500">
 ...
 <!-- Expanded content -->
 <div v-if="expandedProviders[pIdx]" class="space-y-3 border-t border-gray-100 px-4 pb-4 pt-3">
 ...
 <div v-if="provider.quota_limit != null && provider.quota_limit > 0" class="flex-1 rounded-full bg-gray-200" style="height: 6px">
 <div
 class="h-full rounded-full transition-all"
 :class="quotaPercentage(provider) > 90 ? 'bg-red-500' : quotaPercentage(provider) > 70 ? 'bg-yellow-500' : 'bg-primary-500'"
 ...
 <!-- Test results -->
 <div v-if="wsTestResult" class="mt-4 max-h-80 overflow-y-auto rounded-lg bg-gray-50 p-4">
 ...
 <div v-for="(r, rIdx) in wsTestResult.results" :key="rIdx" class="mt-2 border-t border-gray-200 pt-2 first:mt-0 first:border-0 first:pt-0">
 <a :href="r.url" target="_blank" class="text-sm font-medium text-primary-600 hover:underline">{{ r.title }}</a>
 <p class="mt-0.5 text-xs text-gray-500">{{ r.snippet }}</p>
 </div>
 </div>
...
<!-- Tab: Payment -->
 ...
 <div class="card">
 <div class="border-b border-gray-100 px-6 py-4">
 <h2 class="text-lg font-semibold text-gray-900">{{ t('admin.settings.payment.title') }}</h2>
 <p class="mt-1 text-sm text-gray-500">
 {{ t('admin.settings.payment.description') }}
 <a :href="locale === 'zh' ? 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT_CN.md' : 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT.md'" target="_blank" rel="noopener noreferrer" class="ml-2 inline-flex items-center text-primary-600 hover:text-primary-700">
 ...
 <div class="flex items-center justify-between">
 <div>
 <label class="font-medium text-gray-900">{{ t('admin.settings.payment.enabled') }}</label>
 <p class="text-sm text-gray-500">{{ t('admin.settings.payment.enabledHint') }}</p>
 </div>
 ...
 <div><label class="input-label">{{ t('admin.settings.payment.productNamePrefix') }}</label><input v-model="form.payment_product_name_prefix" type="text" class="input" placeholder="芝麻 ZHIMA" /></div>
 ...
 <div><label class="input-label">{{ t('admin.settings.payment.preview') }}</label><div class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-600">{{ (form.payment_product_name_prefix || '芝麻 ZHIMA') + ' 100 ' + (form.payment_product_name_suffix || 'CNY') }}</div></div>
 </div>
 ...
 <p class="mt-1 text-xs font-medium text-primary-600">{{ t('admin.settings.payment.balanceRechargePreview', { usd: (Number(form.payment_balance_recharge_multiplier) || 1).toFixed(2) }) }}</p>
 </div>
 <div>
 ...
 <p v-if="(Number(form.payment_recharge_fee_rate) || 0) > 0" class="mt-1 text-xs font-medium text-primary-600">{{ t('admin.settings.payment.rechargeFeePreview', { fee: (Number(form.payment_recharge_fee_rate) || 0).toFixed(2) }) }}</p>
 </div>
 ...
 form.payment_cancel_rate_limit_enabled ? 'bg-primary-500' : 'bg-gray-300 '
 ]"
 ...
 <span :class="['text-sm whitespace-nowrap', form.payment_cancel_rate_limit_enabled ? 'text-gray-700 ' : 'text-gray-400 ']">{{ t('admin.settings.payment.cancelRateLimitEvery') }}</span>
 ...
 <span :class="['text-sm whitespace-nowrap', form.payment_cancel_rate_limit_enabled ? 'text-gray-700 ' : 'text-gray-400 ']">{{ t('admin.settings.payment.cancelRateLimitAllowMax') }}</span>
 ...
 <span :class="['text-sm whitespace-nowrap', form.payment_cancel_rate_limit_enabled ? 'text-gray-700 ' : 'text-gray-400 ']">{{ t('admin.settings.payment.cancelRateLimitTimes') }}</span>
 </div>
 </div>
 </div>
 ...
 <Icon name="mail" size="md" class="mt-0.5 flex-shrink-0 text-gray-400" />
 <div>
 ...
 <div class="flex items-center justify-between">
 <label class="mb-0 block text-sm font-medium text-gray-700">{{ t('admin.settings.balanceNotify.enabled') }}</label>
 <Toggle v-model="form.balance_low_notify_enabled" />
 </div>
 <div v-if="form.balance_low_notify_enabled">
 <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('admin.settings.balanceNotify.threshold') }}</label>
 <div class="relative">
 <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">$</span>
 <input v-model.number="form.balance_low_notify_threshold" type="number" min="0" step="0.01" class="input pl-7" />
 </div>
 <p class="mt-1 text-xs text-gray-500">{{ t('admin.settings.balanceNotify.thresholdHint') }}</p>
 </div>
 <div>
 <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('admin.settings.balanceNotify.rechargeUrl') }}</label>
 <input v-model="form.balance_low_notify_recharge_url" type="url" class="input" :placeholder="currentOrigin" />
 <p class="mt-1 text-xs text-gray-500">{{ t('admin.settings.balanceNotify.rechargeUrlHint') }}</p>
 </div>
 </div>
 </div>
 ...
 <div class="px-6 py-6 space-y-4">
 <div class="flex items-center justify-between">
 <label class="mb-0 block text-sm font-medium text-gray-700">{{ t('admin.settings.quotaNotify.enabled') }}</label>
 <Toggle v-model="form.account_quota_notify_enabled" />
 </div>
 <div v-if="form.account_quota_notify_enabled">
 <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('admin.settings.quotaNotify.emails') }}</label>
 <div class="space-y-2">
 ...
 <p class="mt-1 text-xs text-gray-500">{{ t('admin.settings.quotaNotify.emailsHint') }}</p>
 </div>
 </div>
 </div>
 </div><!-- /Tab: Email -->
