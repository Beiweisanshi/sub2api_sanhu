<!-- 作者：mkx | 日期：2026-04-21 | 变更：统一 Key Usage 公开页为 Claude 暖色主题并移除暗色切换 -->
<template>
 <div class="relative flex min-h-screen flex-col overflow-hidden bg-claude-bg text-claude-text">
 <div class="pointer-events-none absolute inset-0 overflow-hidden">
 <div class="absolute inset-0 bg-mesh-gradient opacity-90"></div>
 <div class="hero-grid-light opacity-60"></div>
 ...
 <div class="h-10 w-10 overflow-hidden rounded-xl border border-claude-border bg-white shadow-md">
 ...
 <!-- Main Content -->
 <main class="relative z-10 mx-auto flex w-full max-w-5xl flex-1 px-6 py-12">
 <!-- Hero -->
 ...
 class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 transition-all"
 ...
 <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm">
 <span
 class="w-2.5 h-2.5 rounded-full pulse-dot"
 :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-orange-500'"
 ></span>
 ...
 :key="i"
 class="fade-up rounded-2xl border border-gray-200 bg-white/90 p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-lg"
 ...
 class="fade-up fade-up-delay-3 rounded-2xl border border-gray-200 bg-white/90 backdrop-blur-sm overflow-hidden"
 >
 ...
 class="fade-up fade-up-delay-4 rounded-2xl border border-gray-200 bg-white/90 backdrop-blur-sm overflow-hidden"
 >
 <div class="px-8 py-5 border-b border-gray-200">
...
const ringTrackColor = computed(() => '#E8E5DF')
...
const detailRows = computed<DetailRow[]>(() => {
 ...
 if (data.mode === 'quota_limited') {
 if (data.quota) {
 const remainColor = data.quota.remaining <= 0 ? 'text-orange-500'
 ...
 ? (data.remaining <= 0 ? 'text-orange-500' : data.remaining < 10 ? 'text-amber-500' : 'text-primary-500')
 : ''
 rows.push({
 iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_SHIELD,
...
.skeleton {
 background: linear-gradient(90deg, #e8e5df 25%, #f5f3ec 50%, #e8e5df 75%);
