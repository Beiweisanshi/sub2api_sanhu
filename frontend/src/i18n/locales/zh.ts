  ...
  admin: {
    ...
    accounts: {
      ...
      openai: {
        ...
        chatCompletionsNative: 'Chat Completions 原生直通',
        chatCompletionsNativeDesc:
          '仅 API Key 账号可用。开启后请求不再转换为 Responses API，而是直接转发到上游 /v1/chat/completions。适用于 GLM / DeepSeek / Kimi / new-api 等仅实现 /v1/chat/completions 的 OpenAI 兼容上游。',
    ...
    ops: {
      ...
      healthHelp: '基于最终成功率、错误率和资源使用情况的系统整体健康评分',
      healthyStatus: '健康',
      riskyStatus: '风险',
      idleStatus: '待机',
      timeRange: {
      ...
      diagnosis: {
        ...
        healthLowImpact: '可能存在轻度波动，建议关注最终成功率与错误率',
      ...
      runtime: {
        ...
        validation: {
          ...
          slaMinPercentRange: '最终成功率最低值必须在 0-100 之间',
      ...
      settings: {
        ...
        slaMinPercent: '最终成功率最低百分比',
        slaMinPercentHint: '最终成功率低于此值时显示为红色（默认：99.5%）',
        ...
        validation: {
          ...
          slaMinPercentRange: '最终成功率最低百分比必须在0-100之间',
      ...
      tooltips: {
        ...
        errorTrend: '错误趋势：请求错误只统计最终失败；上游错误保留被重试或 failover 覆盖的中间错误，并在上游错误率中排除 429/529。',
        errorDistribution: '按状态码统计的错误分布。',
        upstreamErrors: '上游服务返回的错误，包含被重试或 failover 覆盖的中间错误；上游错误率排除 429/529 限流错误。',
        goroutines:
        ...
        sla: '最终成功率：成功请求 /（成功请求 + 最终请求错误），排除业务限制（如余额不足、配额超限），不把已恢复的中间上游错误算作失败。',
        errors: '错误统计：请求错误表示最终失败，上游错误保留中间错误用于排障。',
        latency: '请求时长统计，包括 p50、p90、p95、p99 等百分位数。',
        ttft: '首 Token 延迟（Time To First Token），衡量流式响应的首 Token 返回速度。',
        health: '系统健康评分（0-100），综合考虑最终成功率、错误率和资源使用情况。'
      },
      charts: {
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 芝麻 ZHIMA 用户登录',
      ...
      gatewayForwarding: {
        ...
        envScrub: '清洗 <env> / 路径',
        envScrubHint: '清洗 system[] 中的 Platform/Shell/OS Version/Working directory 与 $HOME 路径。仅对 OAuth 指纹请求生效。',
        systemReminderScrub: '清洗 <system-reminder>',
        systemReminderScrubHint: '清洗 messages[] 里 <system-reminder>…</system-reminder> 块中的环境字段。仅对 OAuth 指纹请求生效。',
        billingInject: '缺失时注入 billing header',
        billingInjectHint: '请求缺少 x-anthropic-billing-header 时主动注入 canonical 块（配合 CCH 签名使用）。',
        ccFingerprintV2: 'CC 真指纹 (v2)',
        ccFingerprintV2Hint: '用 SHA256 派生的 3-hex 指纹替代 cc_version 后缀。关闭时保留上游原始 cc_version，不做随机化。',
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 芝麻 ZHIMA',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">芝麻 ZHIMA 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是 芝麻 ZHIMA 的核心概念，它就像一个"服务套餐"：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建"VIP专线"（高倍率）和"免费试用"（低倍率）两个分组</p><p style="margin-top: 16px; color: #10b981; font-weight: 600;">👉 点击左侧的"分组管理"开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 芝麻 ZHIMA',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 芝麻 ZHIMA AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
