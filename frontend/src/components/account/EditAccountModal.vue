...
<template>
 ...
 <!-- Usage Percent Limit -->
 <div class="rounded-lg border border-gray-200 p-4">
 <label class="input-label mb-0">{{ t('admin.accounts.quotaControl.usagePercentLimit.label') }}</label>
 <p class="mt-1 text-xs text-gray-500">
 {{ t('admin.accounts.quotaControl.usagePercentLimit.hint') }}
 </p>
 <div class="mt-3 flex items-center gap-2">
 <input
 v-model.number="usagePercentLimit"
 type="number"
 min="0"
 max="100"
 class="input w-32"
 :placeholder="t('admin.accounts.quotaControl.usagePercentLimit.placeholder')"
 />
 <span class="text-sm text-gray-500">{{ t('admin.accounts.quotaControl.usagePercentLimit.unit') }}</span>
 </div>
 </div>
 </div>
...
const usagePercentLimit = ref<number | null>(null)
...
function loadQuotaControlSettings(account: Account) {
 ...
 // Load usage percent limit
 if (account.usage_percent_limit != null && account.usage_percent_limit > 0) {
 usagePercentLimit.value = account.usage_percent_limit
 }
}
...
const handleSubmit = async () => {
 ...
 // Usage percent limit
 if (usagePercentLimit.value != null && usagePercentLimit.value > 0) {
 newExtra.usage_percent_limit = Math.min(100, Math.max(1, Math.round(usagePercentLimit.value)))
 } else {
 delete newExtra.usage_percent_limit
 }
