...
<template>
 ...
 <button
 type="button"
 class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-primary-600 hover:bg-primary-50 transition-colors"
 ...
 <span
 class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-gray-900 px-3 py-2 text-xs leading-relaxed text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100"
 >
...
const geminiTierClass = computed(() => {
 ...
 if (level === 'ultra') return 'bg-gray-100 text-gray-600 '
 if (level === 'pro') return 'bg-primary-100 text-primary-600 '
 return 'bg-gray-100 text-gray-600 '
 }

 if (channel === 'gcp') {
 if (level === 'enterprise') return 'bg-gray-100 text-gray-600 '
 return 'bg-primary-100 text-primary-600 '
 }
