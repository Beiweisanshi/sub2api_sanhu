...
const tierBadgeClass = computed(() => {
 ...
 if (isCodeAssist.value) {
 const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
 if (tier === 'gcp_enterprise') return 'bg-gray-100 text-gray-600 '
 if (tier === 'gcp_standard') return 'bg-primary-100 text-primary-600 '
 // Backward compatibility
 const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
 if (upper.includes('ULTRA') || upper.includes('ENTERPRISE')) return 'bg-gray-100 text-gray-600 '
 return 'bg-primary-100 text-primary-600 '
 }

 if (isGoogleOne.value) {
 const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
 if (tier === 'google_ai_ultra') return 'bg-gray-100 text-gray-600 '
 if (tier === 'google_ai_pro') return 'bg-primary-100 text-primary-600 '
 if (tier === 'google_one_free') return 'bg-gray-100 text-gray-600 '
 // Backward compatibility
 const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
 if (upper === 'GOOGLE_ONE_UNLIMITED') return 'bg-gray-100 text-gray-600 '
 if (upper === 'AI_PREMIUM') return 'bg-primary-100 text-primary-600 '
 return 'bg-gray-100 text-gray-600 '
 }

 // AI Studio 默认样式：蓝色
 const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
 if (tier === 'aistudio_paid') return 'bg-primary-100 text-primary-600 '
