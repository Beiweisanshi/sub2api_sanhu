...
<template>
 ...
 <span v-if="row.group" class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-800">
 {{ row.group.name }}
 </span>
 ...
 <Icon name="arrowDown" size="sm" class="h-3.5 w-3.5 text-primary-500" />
 <span class="font-medium text-gray-900">{{ row.input_tokens?.toLocaleString() || 0 }}</span>
 </div>
 <div class="inline-flex items-center gap-1">
 <Icon name="arrowUp" size="sm" class="h-3.5 w-3.5 text-gray-500" />
 ...
 <span v-if="row.cache_ttl_overridden" :title="t('usage.cacheTtlOverriddenHint')" class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-orange-100 text-orange-600 ring-1 ring-inset ring-orange-200 cursor-help">R</span>
 </div>
 </div>
 </div>
 ...
 >
 <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-gray-100 transition-colors group-hover:bg-primary-100">
 <Icon name="infoCircle" size="xs" class="text-gray-400 group-hover:text-primary-500" />
 </div>
 </div>
 </div>
 </template>
 ...
 <span class="font-medium text-primary-600">${{ row.actual_cost?.toFixed(6) || '0.000000' }}</span>
 <!-- Cost Detail Tooltip -->
 <div
 class="group relative"
 ...
 <div v-if="row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-orange-500">
 ...
 <span v-if="row.first_token_ms != null" class="text-sm text-gray-600">{{ formatDuration(row.first_token_ms) }}</span>
 <span v-else class="text-sm text-gray-400">-</span>
 </template>

 <template #cell-duration="{ row }">
 <span class="text-sm text-gray-600">{{ formatDuration(row.duration_ms) }}</span>
 </template>
 ...
 <span v-if="row.ip_address" class="text-sm font-mono text-gray-600">{{ row.ip_address }}</span>
 <span v-else class="text-sm text-gray-400">-</span>
 </template>
 ...
 <span class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-orange-500/20 text-orange-400 ring-1 ring-inset ring-orange-500/30">R-{{ tokenTooltipData.cache_creation_1h_tokens > 0 ? '5m' : '1H' }}</span>
 </span>
 <span class="font-medium text-orange-400">{{ tokenTooltipData.cache_creation_1h_tokens > 0 ? t('usage.cacheTtlOverridden1h') : t('usage.cacheTtlOverridden5m') }}</span>
 </div>
 ...
 <span class="font-semibold text-primary-400">{{ ((tokenTooltipData?.input_tokens || 0) + (tokenTooltipData?.output_tokens || 0) + (tokenTooltipData?.cache_creation_tokens || 0) + (tokenTooltipData?.cache_read_tokens || 0)).toLocaleString() }}</span>
 </div>
 </div>
 ...
 <span class="font-medium text-gray-300">{{ formatTokenPricePerMillion(tooltipData.output_cost, tooltipData.output_tokens) }} {{ t('usage.perMillionTokens') }}</span>
 </div>
 </template>
 ...
 <span class="font-semibold text-primary-400">{{ formatMultiplier(tooltipData?.rate_multiplier || 1) }}x</span>
 </div>
 <div class="flex items-center justify-between gap-6">
 ...
 <span class="font-semibold text-primary-400">{{ formatMultiplier(tooltipData?.account_rate_multiplier ?? 1) }}x</span>
 </div>
 <div class="flex items-center justify-between gap-6">
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
 ...
 if (requestType === 'stream') return 'bg-primary-100 text-primary-800 '
