    ...
    <form id="model-pricing-form" class="space-y-4" @submit.prevent="handleSubmit">
      <div v-if="isCreate">
        <label class="input-label">
          {{ t('admin.modelPricing.columns.model') }} <span class="text-red-500">*</span>
        </label>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <MultiplierInput v-model="form.fast_multiplier" :label="t('admin.modelPricing.columns.fastMultiplier')" />
      </div>
...
const form = reactive({
  ...
  fast_multiplier: '' as number | string,
  note: ''
})
...
watch(
  ...
  () => {
    ...
    form.fast_multiplier = item?.effective.fast_price_multiplier ?? ''
    form.note = item?.note || ''
  },
  { immediate: true }
)
...
function handleSubmit() {
  ...
  const payload: ModelPricingPayload = {
    ...
    fast_price_multiplier: toOptionalNumber(form.fast_multiplier),
    note: form.note.trim()
  }
  emit('save', modelName, payload, isCreate.value)
}

function toOptionalNumber(value: number | string): number | null {
...
const MultiplierInput = defineComponent({
  name: 'ModelPricingMultiplierInput',
  props: {
    modelValue: { type: [Number, String], default: undefined },
    label: { type: String, required: true }
  },
  ...
  setup(inputProps, { emit: inputEmit }) {
    return () => h('div', [
      h('label', { class: 'input-label' }, [
        inputProps.label,
        h('span', { class: 'ml-1 font-normal text-gray-400' }, 'x')
      ]),
      h('input', {
        value: inputProps.modelValue ?? '',
        type: 'number',
