...
<template>
 ...
 {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-primary-600">${{ totalRecharged.toFixed(2) }}</span>
 </p>
 </div>
 </div>
...
const getIconBg = (item: BalanceHistoryItem) => {
 ...
 if (isSubscriptionType(item.type)) return 'bg-gray-100 '
 return item.value >= 0
 ? 'bg-primary-100 '
 : 'bg-orange-100 '
}
...
const getIconColor = (item: BalanceHistoryItem) => {
 ...
 if (isSubscriptionType(item.type)) return 'text-gray-600 '
 return item.value >= 0
 ? 'text-primary-600 '
 : 'text-orange-600 '
}
