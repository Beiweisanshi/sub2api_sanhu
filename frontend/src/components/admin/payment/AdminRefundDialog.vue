...
<template>
 ...
 <span class="text-gray-600">{{ t('payment.admin.refundRequestedAt') }}</span>
 <span class="text-gray-800">{{ formatDateTime(order.refund_requested_at) }}</span>
 </div>
 <div v-if="order?.refund_request_reason" class="mt-1 text-sm">
 <span class="text-gray-600">{{ t('payment.admin.refundRequestReason') }}:</span>
 <span class="ml-1 text-gray-800">{{ order.refund_request_reason }}</span>
 </div>
 </div>
 ...
 <span class="font-medium text-gray-900">{{ order?.order_type === 'balance' ? '$' : '¥' }}{{ order?.amount?.toFixed(2) }}</span>
 </div>
 <div class="mt-1 flex justify-between text-sm">
 <span class="text-gray-500">{{ t('payment.orders.payAmount') }}</span>
 <span class="font-medium text-gray-900">¥{{ order?.pay_amount?.toFixed(2) }}</span>
 </div>
 ...
 <span class="font-medium text-red-600">{{ order?.order_type === 'balance' ? '$' : '¥' }}{{ actuallyRefunded.toFixed(2) }}</span>
 </div>
 </div>
 ...
 <div class="mt-1 font-semibold text-gray-900">${{ userBalance.toFixed(2) }}</div>
 </div>
 <div class="rounded-lg bg-gray-50 p-3 text-sm">
 <div class="text-gray-500">{{ t('payment.admin.orderAmount') }}</div>
 <div class="mt-1 font-semibold text-gray-900">{{ order?.order_type === 'balance' ? '$' : '¥' }}{{ order?.amount?.toFixed(2) }}</div>
 </div>
 </div>
 ...
 class="mt-2 rounded-lg bg-primary-50 p-3 text-sm text-primary-700"
 >
